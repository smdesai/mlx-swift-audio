...
public class DiffusersAttention: Module {
    ...
    public init(queryDim: Int, heads: Int = 8, dimHead: Int = 64, bias: Bool = false) {
        self.heads = heads
        self.dimHead = dimHead
        self.innerDim = heads * dimHead
        self.scale = pow(Float(dimHead), -0.5)

        self._queryProj.wrappedValue = Linear(queryDim, innerDim, bias: bias)
        self._keyProj.wrappedValue = Linear(queryDim, innerDim, bias: bias)
        self._valueProj.wrappedValue = Linear(queryDim, innerDim, bias: bias)
        self._outProj.wrappedValue = Linear(innerDim, queryDim, bias: bias)
    }
...
public class BasicTransformerBlock: Module {
    ...
    public init(
        ...
        self._attn.wrappedValue = DiffusersAttention(
            queryDim: dim,
            heads: numAttentionHeads,
            dimHead: attentionHeadDim,
            bias: false
        )
        self._ff.wrappedValue = FeedForward(dim: dim, innerDim: dim * 4)
    }

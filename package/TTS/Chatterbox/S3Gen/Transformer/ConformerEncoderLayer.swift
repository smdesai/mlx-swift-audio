...
public class ConformerEncoderLayer: Module {
    ...
    public init(
        ...
        self._feedForwardMacaron.wrappedValue = feedForwardMacaron
        self._convModule.wrappedValue = convModule
        ...
        if feedForwardMacaron != nil {
            self._normFFMacaron.wrappedValue = LayerNorm(dimensions: size, eps: 1e-12)
            self.ffScale = 0.5
        } else {
            self.ffScale = 1.0
        }
        ...
        if convModule != nil {
            self._normConv.wrappedValue = LayerNorm(dimensions: size, eps: 1e-12)
            self._normFinal.wrappedValue = LayerNorm(dimensions: size, eps: 1e-12)
        }
    }
